...
include('../link/desigene/db.php');
            $sql = "SELECT t.*,tr.*,e.* FROM `tabill` AS t INNER JOIN `travelrequest` AS tr ON t.`RequestNoTravel` = tr.`id` INNER JOIN `employeedata` AS e ON t.`EmployeeNo` = e.`EmployeeNo` WHERE tr.`Statusofmanger` = 'ACCEPT' AND tr.`StatusofGM` = 'ACCEPT' AND t.Statusofmanger = 'ACCEPT' AND t.StatusofGM = 'ACCEPT'";
