...
class LB_PDF extends FPDF {
    ...
    // Define getMargins() method to return an array of margins
    ...
    public function Header() {
        if ($this->pageHeaderRepeat || (!$this->pageHeaderRepeat && !$this->pageHeaderAdded)) {
            ...
            $this->SetFont('', 'B', 11);
            $this->Cell(0, 7, 'WATER & SANITATION SERVICES COMPANY MINGORA', 0, 0, 'C');
            $this->Ln();
            $this->SetFont('', 'B', 11);
            $this->Cell(0, 7, 'Govt. of Khyber Pakhtunkhwa', 0, 0, 'C');
            $this->Ln();
            
            $this->SetFont('', 'B', 10);
            $this->Cell(0, 7, 'Main GT Road, Bypass Choke, Qambar, MSK Tower, Mingora, SWAT ', 0, 0, 'C');
            $this->Ln();
            $this->SetFont('', 'B', 10);
            $this->Cell(0, 7, 'Registered Under Section 42 of Companies Act 2017', 0, 0, 'C');
            $this->Ln();
