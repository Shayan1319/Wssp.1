                        ...
                        while ($rowofTP = mysqli_fetch_array($select)) {
                            ...
                            if (isset($_POST["submit" . $a])) {
                                ...
                                foreach ($Ids as $key => $Id) {
                                    $salarySql = mysqli_query($conn, "UPDATE `salary` SET `finace`='{$Status[$key]}', `finaceDate`='$Date' WHERE `id`='$Id'");
                            ...
                            <form action="" method="post">
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="heading<?php echo $a?>">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?php echo $a ?>" aria-expanded="true" aria-controls="collapse<?php echo $a ?>">Time Period # <?php echo $rowofTP['ID'] ?> | From: <?php echo $rowofTP['FromDate'] ?> | To: <?php echo $rowofTP['ToDate'] ?> | Working day: <?php echo $rowofTP['WrokingDays'] ?>
                                        <?php
                                        // Assuming $conn is your database connection
                                        $querySalary = mysqli_query($conn, "SELECT SUM(net_pay) AS total_rate FROM salary WHERE `timeperiod` = $lastTimePeriodId");
                                        while($rowSalary = mysqli_fetch_array($querySalary)){
                                            echo "Total Net Pay for Time Period".$rowSalary['total_rate'];
                                        }                                          
                                        ?>
                                    ...
                                    <div id="collapse<?php echo $a ?>" class="accordion-collapse collapse show" aria-labelledby="heading<?php echo $a ?>" data-bs-parent="#accordionExample">
                                        <div class="accordion-body">
                                            ...
                                            <?php
                                                ...
                                                $selectmang = mysqli_query($conn, "SELECT * FROM `salary` WHERE `employee_id`='$employee' && `timeperiod`='$Timeid' && `HrReview`='ACCEPT' && `finace`='PENDING'&& `InternalAuditor`='ACCEPT'");
