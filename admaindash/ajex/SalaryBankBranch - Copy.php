...
// Query to select distinct Perant values where name is 'SalaryBankBranch'
$select = mysqli_query($conn, "SELECT DISTINCT `Perant` FROM `master` WHERE `name`='SalaryBankBranch'");
...
if (mysqli_num_rows($select) > 0) {
    while ($row = mysqli_fetch_assoc($select)) {
        ...
        <?php
        $selectdrop = mysqli_query($conn, "SELECT * FROM `master` WHERE `name`='SalaryBankBranch' AND `Perant`='" . $row['Perant'] . "'"); 
