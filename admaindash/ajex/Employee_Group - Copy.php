...
// Query to select distinct Perant values where name is 'Employee_Group'
$select = mysqli_query($conn, "SELECT DISTINCT `Perant` FROM `master` WHERE `name`='Employee_Group'");
...
if (mysqli_num_rows($select) > 0) {
    while ($row = mysqli_fetch_assoc($select)) {
        ...
        <hr>
        <?php
        $selectdrop = mysqli_query($conn, "SELECT * FROM `master` WHERE `name`='Employee_Group' AND `Perant`='" . $row['Perant'] . "'"); 
