...
$timeid = $_GET['id'];
$selecttime = mysqli_prepare($conn, "SELECT * FROM `timeperiod` WHERE ID = ?");
mysqli_stmt_bind_param($selecttime, "i", $timeid);
mysqli_stmt_execute($selecttime);
$resultTime = mysqli_stmt_get_result($selecttime);
    ...
    if (mysqli_num_rows($selectemp)) {
      $e = 1;
        while ($rowemp = mysqli_fetch_array($selectemp)) {
            ...
            if ($workingdays > 0 && $attendance_count == $workingdays) {
              $selectsallery=mysqli_query($conn,"SELECT * FROM `salary` WHERE `employee_id`='$employee_id' && `timeperiod`='$timeid'");
              if(mysqli_num_rows($selectsallery)==0){
                ?>
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 table-responsive">
                      <table id="employee_pay<?php echo $e?>" class="table table-bordered table-striped">
                          ...
                          <tbody id="allownces">
                              ...
                              if (mysqli_num_rows($query)) {
                                  ...
                                  while ($fetchdata = mysqli_fetch_assoc($query)) {
                                      ...
                                      else if ($fetchdata['rate'] == 0.00 && $fetchdata['rate_calc_mode'] == 'OFF PAY' && $fetchdata['Weekly_Working_Days']==5) {
                                        $rateInput = 'block';
                                        $rateDis = 'none';
                                        $netPayInput = $fetchdata['net_pay'];
                                          $price = $netPayInput / 22; 
                                        $rateInputvalue = $fetchdata['AbsentCount'];
                                    } else if ($fetchdata['rate'] == 0.00 && $fetchdata['rate_calc_mode'] == 'DOUBLE DUTY') {
                                          $rateInput = 'block';
                                          $rateDis = 'none';
                                          $rateInputvalue = $fetchdata['DoubleDutyCount'];
                                          $price = $fetchdata['price'];
                                      } else if ($fetchdata['rate'] == 0.00 && $fetchdata['rate_calc_mode'] == 'OVERTIME') {
                                          $rateInput = 'block';
                                          $rateDis = 'none';
                          ...
                          ?>
                                      <tr>
                                          <td><?php echo $a?></td>
                                          <td><?php echo $fetchdata['allowance'] ?>
                                              <input style="display: none;" type="text" value="<?php echo $fetchdata['allowances_id'] ?>" name="allowances_id[]" id="allowances_id<?php echo $e?>">
                                              <input style="display: none;" type="text" value="<?php echo $fetchdata['allowance'] ?>" name="allowance[]" id="allowance<?php echo $e?>">
